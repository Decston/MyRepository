...
public class FornecedorDao {
	...
	public List<Fornecedor> getFornecedores() throws SQLException,ClassNotFoundException {
		...
		PreparedStatement ps = conexao.prepareStatement("SELECT CD_FORNECEDOR,NM_FORNECEDOR,"
				+ "DS_RAZÃO_SOCIAL,DS_EMAIL,DS_CNPJ FROM TB_FORNECEDOR");
		ResultSet rs = ps.executeQuery();
		List<Fornecedor> fornecedores = new ArrayList<>();
		while(rs.next()) {
			fornecedores.add(new Fornecedor(rs.getInt(1),rs.getString(4),rs.getString(2),
	...
	public void salvar(Fornecedor fornecedor) throws SQLException,ClassNotFoundException {
		...
		PreparedStatement statement = conexao.prepareStatement(
				"INSERT INTO TB_FORNECEDOR(NM_FORNECEDOR,DS_RAZÃO_SOCIAL,DS_CNPJ,"
				+ "DS_EMAIL) VALUES (?,?,?,?)");
	...
	public void editar(Integer codFornecedor) {
		// TODO Auto-generated method stub
		
	}

	public Fornecedor getFornecedorId(Integer codFornecedor) throws ValidacaoException, ClassNotFoundException, SQLException {
		Connection conexao = ConexaoJDBCFactory.getConexao();
		PreparedStatement ps = conexao.prepareStatement("SELECT CD_FORNECEDOR,NM_FORNECEDOR, "
				+ "DS_RAZÃO_SOCIAL,DS_EMAIL,DS_CNPJ FROM TB_FORNECEDOR WHERE CD_FORNECEDOR = ?");
		ps.setInt(1, codFornecedor);
		ResultSet rs = ps.executeQuery();
		if(rs.next()) {
			return new Fornecedor(rs.getInt(1),rs.getString(4),rs.getString(2),
					rs.getString(3),rs.getString(5));
		}
	...
	public void atualizar(Fornecedor fornecedor) throws SQLException,ClassNotFoundException {
		Connection conexao = ConexaoJDBCFactory.getConexao();
		PreparedStatement statement = conexao.prepareStatement(
				"UPDATE TB_FORNECEDOR SET NM_FORNECEDOR = ?,DS_RAZÃO_SOCIAL = ?,DS_CNPJ = ?,"
